			...
			if(node != null) {
				...
				if(infoWinMrtg.lock==false){
					...
					if(node.item.data.@idx!="")
						iframeMrtg.source=mrtgDirPath+node.item.data.@ip+"/day"+node.item.data.@idx+"minify.html";
	
					iframeDev.source=mrtgDirPath+node.item.data.@ip+"/devicecontent.html";
				//	Alert.show(mrtgDirPath);
				}
